...
#######################################################################################################################
    ...
    async def on_error(self, event, *args, **kwargs):
        ...
        if ex_type == exceptions.HelpfulError:
            ...
            await asyncio.sleep(2)  # don't ask -_-
            await self.logout()
    ...
    async def on_ready(self):
        ...
        # wait_for_message is pretty ewww!!!
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 9506394                          #side bar color

        e.set_footer(text='DarkWhite-WannaPlay ({})'.format(BOTVERSION), icon_url='https://imgur.com/IhztHF7.png')
        e.set_author(name=self.user.name, url='https://github.com/habibrahad/DarkWhite-WannaPlay', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_resetplaylist(self, player, channel):
        ...
        return Response(self.str.get('cmd-resetplaylist-response', '\N{OK HAND SIGN}'), delete_after=15) #change the variable to keep the massage for a long.
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, see https://github.com/habibrahad/DarkWhite-WannaPlay/').format(prefix)
        if not self.is_all:
    ...
    async def cmd_join(self, channel, guild, author, voice_channel):
        """
        Usage:
            {command_prefix}join/something else - which is/are in alieas.json
